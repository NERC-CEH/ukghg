...
install.packages("./ukghg_0.7.0.tar.gz", repos = NULL, type="source")
library(ukghg)
packageVersion("ukghg")
runShinyApp()
vignette(package = "ukghg")
vignette("use_ukghg", package = "ukghg")
browseVignettes("ukghg")
RShowDoc("use_ukghg", package = "ukghg")
?ukghg
?calcFlux
?calcFlux_anthro
?calcFlux_bio
...
?writeNetCDF
?writeGIF
startDate <- as.POSIXct(strptime("01/01/1990 12:00", "%d/%m/%Y %H:%M"), tz = "UTC")
endDate   <- as.POSIXct(strptime("01/12/1990 12:00", "%d/%m/%Y %H:%M"), tz = "UTC")
# create a sequence of dates
nTimes <- 12
datect <- seq(startDate, endDate, length = nTimes)
flux_100 <- calcFlux("c2h6", datect, proj = "OSGB", 
  res = "100", unitType = "g", unitSIprefix = "tera", timeScales = c(TRUE, FALSE, FALSE, FALSE))
plot(datect, flux_100$total, col = "red", type = "b")
## make the two match - how does res^2 affect the units? 
## need to multiply if summing up?
x <- cellStats(flux_100$s_ghgTotal, sum) *km2_to_m2 *secsPerYear *100^2
mean(x) # * 25 # include GWP to compare with original spreadsheet
  # all gases in Tg y-1, converted to Tg m-2 s-1
  value <-  *km2_to_m2 *secsPerYear
