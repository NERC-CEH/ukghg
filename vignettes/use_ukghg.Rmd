...
# output:
  # html_document:
    # keep_md: yes
vignette: >
  %\VignetteIndexEntry{Using ukghg}
...
library(rmarkdown)
system.time(render("use_ukghg.Rmd", output_file = "use_ukghg.html"))
```
...
To predict quarterly ethane emission over 2016 on a 0.01 deg Lon-Lat grid in pg C$_2$H$_6$ m^-2^ s^-1^, ignoring day-of-the-week and diurnal variation, we would use:
```{r}
# create a sequence of quarterly time points over one year
startDate <- as.POSIXct(strptime("01/01/2016", "%d/%m/%Y"), tz = "UTC")
endDate   <- as.POSIXct(strptime("01/10/2016", "%d/%m/%Y"), tz = "UTC")
nTimes <- 4  # number of time points
datect <- seq(startDate, endDate, length = nTimes)

system.time(
  flux_c2h6 <- calcFlux("c2h6", datect, proj = "LonLat", res = "0.01", unitType = "g", unitSIprefix = "pico",
    timeScales = c(TRUE, TRUE, FALSE, FALSE))
)
```
